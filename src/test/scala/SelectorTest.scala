...
trait SelectorTest extends FunSuite {
  ...
  test("test Selector1 selection on data0") {
    assert(Selector1.selectBackDated(data0).sortBy[Int](i => i) == List())
  }

  test("test Selector2 selection on data0") {
    assert(Selector2.selectBackDated(data0).sortBy[Int](i => i) == List())
  }
